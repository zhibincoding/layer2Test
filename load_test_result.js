...
function AnalizeTestResult(filename, rootJson, rootPath) {
  ...
  if (arr == null) {
    Case.fail = 0;
    Case.success = Case.total;
    let cov = 100;
    let percent = cov.toFixed(2) + '%';
    console.log("|", Case.folder, "|", Case.total, "|", Case.success, '|', Case.fail - Case.known, '|', Case.known, '|', percent);
    return;
  }
  Case.fail = arr.length;
  Case.success = Case.total - Case.fail;
  forkname = "";
  ...
  for (i = 0; i < arr.length; i++) {
    ...
    if (rootPath.indexOf('scroll') >= 0) {
      ...
      if (lowerCase.indexOf('suicide') >= 0 || lowerCase.indexOf("selfdestruct") >= 0 || lowerCase.indexOf('deletedaccount') >= 0) {
        ...
        Case.known++;
        Case.reason = scrollJson[failCase.casename];
      }
    }
  ...
  let cov = 100 * (Case.success) / (Case.total - Case.known);
  let percent = cov.toFixed(2) + '%';
  console.log("|", Case.folder, "|", Case.total, "|", Case.success, '|', Case.fail - Case.known, '|', Case.known, '|', percent);
